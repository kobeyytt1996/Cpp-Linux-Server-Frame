 ...
 * @file config.h
 * @author Yuan Yuan
 * @brief 用于配置模块。设计思想为约定优于配置。声明一个配置的时候，已经设置好了默认参数。通常情况，用系统默认参数就够了。必要时，用yaml可以灵活的改其中部分配置
 * 定义了约定项的类，和存储约定项的管理类，以便加载配置文件来修改相应的约定项
 * 支持基础数据类型，也支持stl容器，还支持自定义数据类型（但需要实现序列化和反序列化方法）
 * 为什么用YAML格式的配置文件：在配置文件的场景下，比XML轻量级，也比json更清晰，没有那么多引号
 * 想换为xml或json也很简单，修改序列化和反序列化的方式即可
 */
