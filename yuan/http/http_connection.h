 ...
 * 
 * 简化使用http请求的过程。正常过程：输入url，解析域名，找到远端socket的地址，建立连接，
 * 再根据url的内容并设置头部、请求体等来生成Http请求报文，请求后再获取响应。
 * 
 * 还包含了连接池
 */
...
// 重点：为了处理长连接，使用了连接池的概念。使用完连接，根据某些规则判断是否需要放回连接池以待再次使用
// 在高并发的情况下，长连接能优化性能，尤其针对https更是。减少建立连接消耗的时间
